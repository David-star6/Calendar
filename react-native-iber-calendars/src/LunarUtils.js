...
(function () {
    ...
    var DATA = {
        ...
        solarTerm: ['小寒', '大寒', '立春', '雨水', '驚蟄', '春分', '清明', '穀雨', '立夏', '小滿', '芒種', '夏至', '小暑', '大暑', '立秋', '處暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪', '冬至'], //二十四节气
	...
	*/
    ...
    function isThanksgiving(_year, _month, _day) {
        //计算2017年的感恩节  
    ...
    // 復活節
    function isEasterDay(year, cmonth, cday) {

        for (let i = 0; i < passionDay.length; i++) {

            let item = passionDay[i];
            let theDay = item.day;
            let nextDay = 1
            let nextMonth = item.month
            if (theDay == 29) {
                // 30天的月份 下兩天
                if (Day_31.indexOf(item.month) == -1) {
                    nextDay = 1
                    nextMonth = item.month + 1
                } else {
                    nextDay = 31
                }
            } else if (theDay == 30) {
            ...
            if (item.year == year && nextMonth == cmonth && nextDay == cday) {
                return true;
            }
        ...
        // return month == cmonth && day == cday ? true : false;
    };

    // 復活節補假
    function isEasterDayNextDay(year, cmonth, cday) {
        ...
        // return month == cmonth && day + 1 == cday ? true : false;
    }


    // 耶穌受難日
    function isPassionSunday(year, cmonth, cday) {

        for (let i = 0; i < passionDay.length; i++) {
            let item = passionDay[i];
            if (item.year == year && item.month == cmonth && item.day == cday) {
                return true;
            }
        }
        return false
        ...
        // var pday = day - easterDay.getDay() - 2;
        // easterDay.setFullYear(year, month - 1, pday);

        // //0~ 6
        // return (easterDay.getMonth() + 1) == cmonth && easterDay.getDate() == cday ? true : false;
    };

    // 耶稣受难日翌日
    function isPassionSundayNextDay(year, cmonth, cday) {
        ...
        // return (easterDay.getMonth() + 1) == cmonth && easterDay.getDate() + 1 == cday ? true : false;
    }
    ...
    function getTermStartName(_year, _month, _day) {
        _month = _month + 1;
        let target = _year + '-' + (_month >= 10 ? _month : ('0' + parseInt(_month))) + '-' + (_day >= 10 ? _day : ('0' + parseInt(_day)));
        if (!global.getJQInfo) {
            global.getJQInfo = [];
        }
        let list = null;
        if (global.getJQInfo[_year]) {
            list = global.getJQInfo[_year];
        } else {
            list = getJQInfo(_year);
            global.getJQInfo[_year] = list;
        }
        ...
        if (info.indexOf(target) != -1) {
            return name[info.indexOf(target)];
        } else {
            return '';
        }
    }
    ...
    function solarToLunar(_year, _month, _day) {
        ...
        var ism = isMonthersDay(date);

        var isf = isFatherDay(date);

        var ise = isEasterDay(_year, _month, _day);

        var iseNext = isEasterDayNextDay(_year, _month, _day);

        var isp = isPassionSunday(_year, _month, _day);

        var ispNext = isPassionSundayNextDay(_year, _month, _day);

        var ist = isThanksgiving(_year, _month, _day);
        ...
        if (ist) {
            ...
            solarFes = solarFestival[formateDayD4(month, day)]
        }
        ...
        var res = {
            ...
            term: getTermStartName(year, month, day),
